...
Use AthenaAgent programmatically — load config, run the agent, get results.
...
### `AthenaAgent.from_config(config_path?, *, workspace?)`

Create an `AthenaAgent` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.athena-agent/config.json`. |
...
```python
from athena_agent.agent import AgentHook, AgentHookContext
...
async def main():
    bot = AthenaAgent.from_config(workspace="/my/project")
