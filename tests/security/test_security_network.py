"""Tests for athena_agent.security.network — SSRF protection and internal URL detection."""
...
def test_blocks_private_ipv4(ip: str, label: str):
    with patch("athena_agent.security.network.socket.getaddrinfo", _fake_resolve("evil.com", [ip])):
...
def test_blocks_ipv6_loopback():
    ...
    with patch("athena_agent.security.network.socket.getaddrinfo", _resolver):
...
def test_detects_curl_metadata():
    with patch("athena_agent.security.network.socket.getaddrinfo", _fake_resolve("169.254.169.254", ["169.254.169.254"])):
        assert contains_internal_url('curl -s http://169.254.169.254/computeMetadata/v1/')


def test_detects_wget_localhost():
    with patch("athena_agent.security.network.socket.getaddrinfo", _fake_resolve("localhost", ["127.0.0.1"])):
...
def test_whitelist_does_not_affect_other_blocked():
    ...
    configure_ssrf_whitelist(["100.64.0.0/10"])
    try:
        with patch("athena_agent.security.network.socket.getaddrinfo", _fake_resolve("evil.com", ["10.0.0.1"])):
