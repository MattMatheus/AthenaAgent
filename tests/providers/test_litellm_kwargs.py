...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("athena_agent.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/foundry/AthenaAgent"
    assert headers["X-OpenRouter-Title"] == "athena-agent"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,agent-runtime"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("athena_agent.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://athena-agent.dev",
                "X-OpenRouter-Title": "AthenaAgent Pro",
    ...
    headers = MockClient.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://athena-agent.dev"
    assert headers["X-OpenRouter-Title"] == "AthenaAgent Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,agent-runtime"
...
async def test_openai_compat_stream_watchdog_returns_error_on_stall(monkeypatch) -> None:
    monkeypatch.setenv("ATHENA_AGENT_STREAM_IDLE_TIMEOUT_S", "0")
