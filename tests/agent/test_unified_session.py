...
from athena_agent.command.builtin import cmd_new, register_builtin_commands
from athena_agent.command.router import CommandContext, CommandRouter
from athena_agent.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("athena_agent.agent.loop.SessionManager"), \
         patch("athena_agent.agent.loop.SubagentManager") as MockSubMgr, \
         patch("athena_agent.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates athena_agent onboard)."""
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from athena_agent.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from athena_agent.agent.loop import UNIFIED_SESSION_KEY
        from athena_agent.command.builtin import cmd_stop
