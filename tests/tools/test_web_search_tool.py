...
from athena_agent.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="athena_agent", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("athena_agent.agent.tools.web.DDGS", MockDDGS, raising=False)
    import athena_agent.agent.tools.web as web_mod
