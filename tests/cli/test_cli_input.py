...
from athena_agent.cli import commands
from athena_agent.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("athena_agent.cli.commands._PROMPT_SESSION", mock_session), \
         patch("athena_agent.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("athena_agent.cli.commands.PromptSession") as MockSession, \
         patch("athena_agent.cli.commands.FileHistory") as MockHistory, \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("athena_agent.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
...
def test_response_renderable_without_metadata_keeps_markdown_path():
    help_text = "🐈 AthenaAgent commands:\n/status — Show bot status\n/help — Show available commands"
