...
from athena_agent.cli.commands import _make_provider, app
from athena_agent.config.schema import Config
from athena_agent.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("athena_agent.config.loader.get_config_path") as mock_cp, \
         patch("athena_agent.config.loader.save_config") as mock_sc, \
         patch("athena_agent.config.loader.load_config") as mock_lc, \
         patch("athena_agent.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "AthenaAgent is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from athena_agent.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "athena_agent.cli.onboard.run_onboard",
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'athena-agent agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"athena-agent status --config {resolved_config}" in compact_output
...
def test_make_provider_uses_github_copilot_backend():
    from athena_agent.cli.commands import _make_provider
    from athena_agent.config.schema import Config
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("athena_agent.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("athena_agent.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("athena_agent.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("athena_agent.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("athena_agent.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("athena_agent.cli.commands._make_provider", return_value=object()), \
         patch("athena_agent.cli.commands._print_agent_response") as mock_print_response, \
         patch("athena_agent.cron.runtime.make_cron_runtime_callback", return_value=AsyncMock()) as mock_cron_callback, \
         patch("athena_agent.bus.queue.MessageBus"), \
         patch("athena_agent.cron.service.CronService") as mock_cron_cls, \
         patch("athena_agent.agent.loop.AgentLoop") as mock_agent_loop_cls:
        agent_loop = MagicMock()
        agent_loop.output_config = None
        ...
        cron = mock_cron_cls.return_value
        ...
        yield {
            ...
            "cron_cls": mock_cron_cls,
            "cron": cron,
            "cron_callback": mock_cron_callback,
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "athena_agent.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("athena_agent.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("athena_agent.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("athena_agent.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("athena_agent.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("athena_agent.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("athena_agent.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("athena_agent.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("athena_agent.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("athena_agent.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("athena_agent.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("athena_agent.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("athena_agent.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("athena_agent.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("athena_agent.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("athena_agent.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("athena_agent.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("athena_agent.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("athena_agent.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("athena_agent.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "athena_agent.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def test_agent_wires_cron_runtime_callback_and_lifecycle(mock_agent_runtime):
    result = runner.invoke(app, ["agent", "-m", "hello"])

    assert result.exit_code == 0
    mock_agent_runtime["cron_callback"].assert_called_once_with(mock_agent_runtime["agent_loop"])
    assert mock_agent_runtime["cron"].on_job is mock_agent_runtime["cron_callback"].return_value
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from athena_agent.cli.commands import _migrate_cron_store
    ...
    with patch("athena_agent.config.paths.get_cron_dir", return_value=legacy_dir):
