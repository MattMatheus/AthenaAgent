...
from athena_agent.cron.types import (
    CronJob,
    CronJobState,
    CronPayload,
    CronRunRecord,
...
def _normalize_job_result(result: Any) -> tuple[Literal["ok", "error", "skipped"], str | None]:
    """Normalize cron callback return values into persisted job status."""
    if result is None:
        return "ok", None
    if result is False or result == "skip" or result == "skipped":
        return "skipped", None
...
class CronService:
                ...
                for j in data.get("jobs", []):
                    jobs.append(CronJob(
                        ...
                        payload=CronPayload(
                            ...
                            to=j["payload"].get("to"),
                            supervisor=(
                                CronSupervisorSpec(**j["payload"]["supervisor"])
                                if j["payload"].get("supervisor")
                                else None
                            ),
                        ),
                        state=CronJobState(
            ...
            "jobs": [
                {
                    ...
                    "payload": {
                        ...
                        "to": j.payload.to,
                        "supervisor": asdict(j.payload.supervisor) if j.payload.supervisor else None,
                    },
                    "state": {
    ...
    async def _execute_job(self, job: CronJob) -> None:
        ...
        try:
            ...
            job.state.last_status, job.state.last_error = _normalize_job_result(result)
            if job.state.last_status == "skipped":
                logger.info("Cron: job '{}' skipped", job.name)
            else:
    ...
    def add_supervisor_job(
        ...
        *,
        ...
        """Add a supervisor-style recurring job."""
        _validate_schedule_for_add(schedule)
        now = _now_ms()
        ...
        logger.info("Cron: added supervisor job '{}' ({})", name, job.id)
        return job
    ...
    def update_job(
        ...
        *,
        ...
        supervisor: CronSupervisorSpec | None | Literal[...] = ...,
        ...
        if supervisor is not ...:
            job.payload.supervisor = supervisor
            if supervisor is not None:
                job.payload.kind = "supervisor_turn"
