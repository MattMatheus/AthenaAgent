"""Interactive onboarding questionnaire for AthenaAgent."""
...
from athena_agent.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from athena_agent.config.loader import get_config_path, load_config
from athena_agent.config.schema import Config, MCPServerConfig
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]AthenaAgent[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from athena_agent.config.schema to get
...
_SETTINGS_SECTIONS: dict[str, tuple[str, str, set[str] | None]] = {
    "Agent Settings": (
        "Agent Defaults",
        "Configure the default runtime model and behavior. Dream settings are optional.",
        None,
    ),
    "Scheduler": (
        "Scheduler Defaults",
        "Configure recurring supervisor defaults for scheduled runtime work.",
        None,
    ),
    "Tools": (
        "Tools Settings",
        "Configure tool profiles, web search, shell exec, and MCP servers",
        {"mcp_servers"},
    ),
}
...
_SETTINGS_GETTER = {
    ...
    "Scheduler": lambda c: c.scheduler.supervisor,
    "Tools": lambda c: c.tools,
}

_SETTINGS_SETTER = {
    "Agent Settings": lambda c, v: setattr(c.agents, "defaults", v),
    "Scheduler": lambda c, v: setattr(c.scheduler, "supervisor", v),
...
def _questionary_select_value(prompt: str, choices: list[str], default: str | None = None) -> str | None:
    """Run a standard questionary select prompt and return the answer."""
    return _get_questionary().select(prompt, choices=choices, default=default, qmark=">").ask()


def _questionary_text_value(prompt: str, default: str = "") -> str | None:
    """Run a standard questionary text prompt and return the answer."""
    return _get_questionary().text(prompt, default=default).ask()


def _configure_obsidian_mcp(config: Config) -> None:
    """Configure the Obsidian MCP server using a guided shortcut."""
    console.clear()
    existing = config.tools.mcp_servers.get("obsidian")
    existing_env = existing.env if existing else {}

    mode_choices = ["Built binary"]
    if existing is not None:
        mode_choices.append("Remove existing Obsidian MCP server")
    mode_choices.append("<- Back")

    subtitle = (
        "Write an `obsidian` MCP server entry into tools.mcpServers "
        "using an external obsidianMCP binary."
    )
    _show_section_header("Obsidian MCP", subtitle)

    default_mode = "Built binary"

    mode = _questionary_select_value(
        "How should AthenaAgent launch obsidianMCP?",
        mode_choices,
        default=default_mode if default_mode in mode_choices else mode_choices[0],
    )
    if mode is None or mode == "<- Back":
        return

    if mode == "Remove existing Obsidian MCP server":
        config.tools.mcp_servers.pop("obsidian", None)
        console.print("[green]+ Removed `obsidian` MCP server configuration[/green]")
        return

    default_vault = existing_env.get("OBSIDIAN_VAULT", "SharedKnowledge")
    default_config = existing_env.get(
        "OBSIDIAN_CONFIG",
        str((Path.home() / ".obsidianmcp" / "obsidianmcp.json").expanduser()),
    )
    default_rpc_url = existing_env.get("OBSIDIAN_RPC_URL", "http://127.0.0.1:27124")
    default_timeout = existing_env.get("OBSIDIAN_COMMAND_TIMEOUT", "1m0s")

    vault = _questionary_text_value("Default vault name:", default=default_vault)
    if not vault:
        return
    obsidian_config = _questionary_text_value("Path to obsidianmcp.json:", default=default_config)
    if not obsidian_config:
        return
    rpc_url = _questionary_text_value("Obsidian RPC URL:", default=default_rpc_url)
    if not rpc_url:
        return
    timeout = _questionary_text_value("Command timeout:", default=default_timeout)
    if not timeout:
        return
    ...
    env = {
        ...
        "OBSIDIAN_RPC_URL": rpc_url,
        "OBSIDIAN_COMMAND_TIMEOUT": timeout,
    }

    command_default = existing.command if existing and existing.command != "go" else "/absolute/path/to/obsidianMCP"
    command = _questionary_text_value("Path to obsidianMCP binary:", default=command_default)
    if not command:
        return
    args_default = " ".join(existing.args) if existing and existing.command != "go" else "mcp"
    args_value = _questionary_text_value("Arguments:", default=args_default)
    if args_value is None:
        return
    args = [part for part in args_value.split() if part]
    ...
    config.tools.mcp_servers["obsidian"] = server
    console.print("[green]+ Configured `obsidian` MCP server[/green]")
...
def _show_summary(config: Config) -> None:
    ...
    for title, model in [
        ...
        ("Scheduler", config.scheduler.supervisor),
        ("Tools", config.tools),
    ]:
...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    ...
    while True:
        ...
        _MENU_DISPATCH = {
            ...
            "[S] Scheduler": lambda: _configure_general_settings(config, "Scheduler"),
            "[T] Tools": lambda: _configure_general_settings(config, "Tools"),
            "[O] Obsidian MCP": lambda: _configure_obsidian_mcp(config),
            "[V] View Configuration Summary": lambda: _show_summary(config),
        }

        if answer == "[W] Save and Exit":
            return OnboardResult(config=config, should_save=True)
        if answer == "[X] Exit Without Saving":
            return OnboardResult(config=original_config, should_save=False)
