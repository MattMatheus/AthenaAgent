...
from athena_agent.providers.openai_responses import (
    consume_sdk_stream,
    convert_messages,
    convert_tools,
...
if TYPE_CHECKING:
    from athena_agent.providers.registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/foundry/AthenaAgent",
    "X-OpenRouter-Title": "athena-agent",
    "X-OpenRouter-Categories": "cli-agent,agent-runtime",
}
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply AthenaAgent attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("ATHENA_AGENT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    body = self._build_responses_body(
