...
If you discover a security vulnerability in athena_agent, please report it by:
...
chmod 600 ~/.athena-agent/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.athena-agent/config.json` with file permissions set to `0600`
...
- ✅ Never run athena_agent as root
...
- Config files and API keys (`~/.athena-agent/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run athena_agent with a dedicated user account
...
- Keep authentication data in `~/.athena-agent/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade athena_agent-ai
```
...
- Subscribe to security advisories for athena_agent and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install athena_agent-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash athena_agent
   sudo -u athena_agent athena_agent gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.athena-agent
   chmod 600 ~/.athena-agent/config.json
   chmod 700 ~/.athena-agent/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.athena-agent/logs/athena_agent.log
   ```
...
- **Chat history is stored locally** - protect the `~/.athena-agent` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.athena-agent/logs/athena_agent.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/athena_agent/security/advisories
- Release Notes: https://github.com/HKUDS/athena_agent/releases
